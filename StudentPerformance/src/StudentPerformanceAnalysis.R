yield_data <- read.csv("C:/Users/Bhavana/Downloads/StudentsPerformance.csv")
...
clean_yield_data<-read.csv2("D:/PDS/Bhavana/StudentPerformance/data_clean/clean_yield_data.csv")
clean_yield_data
library(tidyverse) # metapackage with lots of helpful functions
library(dplyr)
library(ggplot2)
data=read.csv("C:/Users/Bhavana/Downloads/StudentsPerformance.csv")
...
ggsave("D:/PDS/Bhavana/StudentPerformance/results/race.ethnicity.jpg")
total <- table(data$race.ethnicity)
plot(table(data$race.ethnicity))
ggsave("D:/PDS/Bhavana/StudentPerformance/results/data$race.ethnicity.jpg")
x1 <- data %>% 
  group_by(race.ethnicity) %>%
  filter(math.score > 80) 
table(x1$race.ethnicity)
x1 %>% ggplot() + geom_bar(aes(x1$race.ethnicity,fill = x1$race.ethnicity))
ggsave("D:/PDS/Bhavana/StudentPerformance/results/x1$race.ethnicity.jpg")
data %>% 
  group_by(gender) %>%
...
ggsave("D:/PDS/Bhavana/StudentPerformance/results/Gender.jpg")
level1 <- data %>% 
...
ggsave("D:/PDS/Bhavana/StudentPerformance/results/Education level chart.jpg")
